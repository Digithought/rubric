...
3. If no `FEATURE:` cross-reference exists, search for terms that match `capabilities` to find probable implementation. Confirm the match is real (function names, file purpose, integration with the surrounding system) — do not be fooled by lexical coincidence.
...
- **gap (cross-ref missing)** — code clearly implements the feature, but no `FEATURE:` comment marks it. File a smaller ticket asking only for the cross-reference.
...
Use the ticket template. Ensure the ticket includes:
- The feature code and name.
- Whether the gap is "no code" or "missing cross-reference" or "partial".
- The file paths you searched and the term/identifier you would expect.
- A pointer to the feature file.
