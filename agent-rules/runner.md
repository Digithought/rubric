...
- **Ledger.** `current` and `all` runs write the coverage ledger. A later-release run does not, and says so once, because the ledger records current coverage; for the same reason `--stale-only` is refused with a later release. A record's `feature-hash` does not depend on the run's target, so its freshness means the same whichever run wrote it, and the later write wins. An `all` run's verdict also covers the feature's later capabilities, so a gap it records may concern only later work.
